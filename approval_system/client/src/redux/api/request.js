...
export const requestApiSlice = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    userTicketsPage: builder.query({
      query: ({id,page}) => `/api/ticket/user-tickets/${id}/${page}`
    }),
    //user-tickets
    userTickets: builder.query({
      query: (id) => `/api/ticket/user-tickets/${id}`
    }),
    //open new tickets
    ticketOpen: builder.mutation({
      query: ({id,userId}) => ({
        url: `/api/ticket/ticket-open/${id}/${userId}`,
        method: "PUT"
      })
    }),
    //aom-tickets
    aomTickets: builder.query({
      query: (id) => `/api/ticket/aom-tickets/${id}`
    }),
    //aom-tickets with page
    aomTicketsPage: builder.query({
      query: ({page,id}) => `/api/ticket/aom-tickets/${id}/${page}`
    }),
    //ticket-declined
    ...
    search: builder.query({
      query: ({page, query, id}) => `/api/ticket/ticket-search/${id}/${page}?status=${query}`
    }),
    //max pages search
    maxPagesSearch: builder.query({
      query: ({query,id}) => `/api/ticket/ticket-maxpage-search/${id}?status=${query}`
    }),
    //report
