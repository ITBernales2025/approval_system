...
export const Accounts = () => {
  ...
  const {data: allUsersQuery,refetch:allUsersRefetch, error} = useAllUsersQuery()
  const dispatch = useDispatch()
  const [successUpdate, setSuccessUpdate ] = useState(false)
  const [isOnline, setIsOnline] = useState(false)
  const [search, setSearch] = useState('')
  const [resetPass, setResetPass] = useState(false)
  const searchFilter = allUsersQuery?.filter((e)=> e.name.includes(search))
  

  useEffect(()=> {
    dispatch(setAllUsers(searchFilter))
  },[search])

  useEffect(()=> {
    allUsersRefetch()
    const intervalRefetch = setInterval(allUsersRefetch, 3000)
