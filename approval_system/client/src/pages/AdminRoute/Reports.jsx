...
export const Reports = () => {
  ...
  const {data: allTickets,isError:allTicketError} = useAllTicketsQuery()
  const {data: reportsData, refetch, isError} = useReportQuery({branch: data.branch, dept: data.dept, from: data.from, to: data.to, page: reportPage})
  ...
  useEffect(()=> {
    if(isError || allTicketError){
      setQueryParams('error',true)
    }
  },[isError,allTicketError])
            ...
            <div className='flex justify-between'>
              ...
              <div className='flex gap-10'>
                <select 
                  id='branch'
                  name='branch'
                  className='border-2 rounded w-52 ' 
