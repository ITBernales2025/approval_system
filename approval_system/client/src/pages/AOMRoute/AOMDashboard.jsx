...
export const AOMDashboard = () => {
  ...
  const {data: aomTicketData, refetch: aomTicketsRefetch, isError} = useAomTicketsQuery(userInfo._id)
  const {data:aomTicketDataWithPage, refetch:aomTicketPageRefetch, isLoading:aomTicketPageLoading, isError:aomTicketError} = useAomTicketsPageQuery({page: ticketPage, id: userInfo._id})
  const {data: maxPageSearch,refetch: maxPagesRefetch, isError:pagesError} = useMaxPagesSearchQuery({query: search, id: userInfo._id})
  ...
  //for search======================================================
  useEffect(()=> {
    if(isError || aomTicketError || pagesError || searchError) {
      setQueryParams('error',true)
    }
  },[isError,aomTicketError,pagesError,searchError])
            ...
            <RequestInformationModal ticket={ticket}>
              {
                ...
                <div className="border-b-4 flex ">
                  ...
                  <textarea value={comment} id="aomComments" name="aomComments" onChange={(e)=> setComment(e.target.value)} className={`w-full p-3 resize-none h-44 ${declinedError && "border border-red-500"}`} placeholder="Write comment here...">
                        </textarea>
                </div>
              }
              {
