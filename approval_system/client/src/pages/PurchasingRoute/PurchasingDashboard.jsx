...
export const PurchasingDashboard = () => {
  ...
  const {data: searchQuery, refetch: searchRefetch, isLoading:searchLoading, isError:searchError} = useSearchQuery({page: ticketPage, query: search, id: userInfo._id})
  const {data: allTicketsPage , refetch:allTicketPageRefetch, isLoading:allTicketPageLoading, isError:ticketPageError} = useAllTicketsWithPageQuery(ticketPage)
  const {data: allTickets, refetch:allTicketsRefetch, isError:allTicketError} = useAllTicketsQuery()
  const {data: maxPageSearch,refetch:maxPagesRefetch, isError:maxPageError} = useMaxPagesSearchQuery({query: search, id: userInfo._id})
  ...
  const handleOpen = async(id, data) => {
    ...
    try {
      await ticketOpen({id: id, userId: userInfo._id})
  ...
  useEffect(()=> {
    if(searchError || ticketPageError || allTicketError|| maxPageError) {
      setQueryParams('error',true)
    }
  },[searchError, ticketPageError, allTicketError, maxPageError])
  ...
  const handleSubmitOos = async()=> {
    try {
      ...
      if(!res.error) {
        setIsOpen(false)
        setPurchasingOosConfirmation(false)
        setTransition("-translate-y-5 opacity-0")
        setOosSuccess(true)
        setComment("")
        allTicketPageRefetch()
        ...
        <div className="absolute left-0 top-0 bg-white/50 w-full h-full z-50 flex items-center justify-center">
          <div className={`xs:w-full lg:w-8/12 xl-6/12 h-[90%] flex flex-col rounded-md shadow-lg shadow-black/50 bg-white relative overflow-hidden transition-all duration-700 ${transition}`}>
            ...
            <RequestInformationModal ticket={ticket}>
                ...
                <div className="flex border-b-4">
                  ...
                  <div className={`w-full p-3 h-96 ${updateOosComment && "border border-orange-500"}`}>
                    { !updateOosComment ? (
                        ...
                        <textarea className={`w-full outline-none resize-none h-full ${errorOosComment? "border border-red-500": ""}`}
                        id="OosComment" name="OosComment"
                        placeholder="Enter new comments " value={oosComment} onChange={(e)=>setOosComment(e.target.value) }></textarea>
                      )
                    }
                  </div>
                </div>
              ...
              {
                !ticket.last_approval_details?.last_approval && !ticket.fraud_details?.is_fraud &&  (!ticket.purchasing_approval?.new_items && !ticket.purchasing_approval?.used_items && ticket.aom_approval_details?.is_approve_aom) && ticket.request === "HANDSET" &&
                <div className="border-b-4 flex ">
                  <div className="w-full p-3  border border-r-4 font-bold">Comments :</div>
                  <textarea value={comment} onChange={(e)=> setComment(e.target.value)} className={`w-full p-3 border resize-none h-44 ${error && "border-red-500"} `} id="aomCommentsApproval" name="aomCommentsApproval"  placeholder="Write comment here...">
                  </textarea>
                </div>
              }
              {
                !ticket.last_approval_details?.last_approval && (!ticket.purchasing_approval?.new_items && !ticket.purchasing_approval?.used_items && ticket.aom_approval_details?.is_approve_aom) && ticket.request === "HANDSET" && !ticket.fraud_details?.is_fraud && (
