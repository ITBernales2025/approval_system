...
export const Dashboard = () => {
  ...
  const {data: userTicketsPage, refetch, isLoading:userTicketsLoading, isError:userTicketsPageError} = useUserTicketsPageQuery({page:ticketPage, id: userInfo._id})
  ...
  const {data: userTicket, refetch:userTickets, isError:userTicketsError} = useUserTicketsQuery(userInfo._id)
  ...
  useEffect(()=> {
    if(userTicketsPageError || maxPageError || userTicketsError || searchError) {
      setQueryParams('error',true)
    }
  },[userTicketsPageError, maxPageError, userTicketsError, searchError])
