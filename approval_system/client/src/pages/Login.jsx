    ...
    <div style={{backgroundImage: `url(/001.jpg)`}} className="w-full mix-blend-lighten min-h-screen bg-fixed bg-no-repeat bg-cover  flex items-center justify-center relative" >
        ...
        {!isLoading ? (<> <div className={`  lg:w-9/12 2xl:w-7/12 min-h-[600px] grid lg:grid-cols-2 bg-cover z-20 rounded-md shadow-lg shadow-black/80 overflow-hidden font-sans transition-all duration-1000 ease-in-out ${transition}`}  style={{backgroundImage: `url(/005.jpg)`}}>
          ...
          <div className=" bg-white w-full flex items-center justify-center flex-col px-16 border gap-5">
            ...
            <h1 className="xs:text-xl lg:text-4xl font-bold text-blue-800">Approval System</h1>
            <p className="xs:text-md lg:text-2xl font-bold text-blue-800 ">LOGIN</p>
            {
              required && 
            ...
            <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
              <label>
                ...
                <input type="text" id="username" name="username" autoComplete="username" className="w-full p-2 border rounded border-blue-900  focus:border-2 focus:outline-none" placeholder="Enter Username" value={username} onChange={(e) => setUsername(e.target.value)}/>
              </label>
              <label className="relative" htmlFor="password">
                <span className="font-semibold text-lg">Password: </span>
                <input type={passShow ? "text" : "password"} id="password" className="w-full p-2 border rounded border-blue-900  focus:border-2 outline-none" placeholder="Enter Password" value={password} onChange={(e)=> setPassword(e.target.value)}/>
                  {
                    passShow ? (
