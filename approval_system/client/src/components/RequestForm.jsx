...
export const RequestForm = ({close, open, success}) => {
  ...
  const {data, refetch, isError} = useUserTicketsPageQuery({page: ticketPage,id: userInfo._id})
  ...
  const handleSubmitData = async()=> {
    try { 
      const res = await newRequest({data: requestForm, id: userInfo._id})
      if(!res.error) {
        setRequestFrom({
          employee_name: "",
          position: "",
      ...
      <div className={`w-4/12 bg-white min-h-5/6 py-5 shadow-black/10 shadow-lg rounded-md flex items-center justify-center flex-col gap-5 relative duration-700 transition-all ease-in-out ${openForm} `}>
        ...
        <form className="w-full 10 flex flex-col gap-2 px-20" onSubmit={handleSubmit}>
          ...
          <label>
            ...
            <input type="text" value={requestForm.employee_name} className="w-full border rounded border-blue-500 px-2 py-1 focus:outline-blue-800" id="employeeName" name="employeeName"  onChange={(e)=> setRequestFrom({...requestForm, employee_name: e.target.value})} />
          </label>
          <label>
            <span className="font-semibold">Position Title :</span>
            <input type="text" id="position" name="position" value={requestForm.position} className="w-full border rounded border-blue-500 px-2 py-1 focus:outline-blue-800" onChange={(e)=> setRequestFrom({...requestForm, position: e.target.value})}/>
          </label>
    
          <label>
            <span className="font-semibold">Request :</span>
            <select className="w-full border rounded border-blue-500 px-2 py-1 focus:outline-blue-800" id="request" name="request" onChange={(e)=> setRequestFrom({...requestForm, request: e.target.value})}>
          ...
          <label>
            ...
            <input type="number" min={1} id="quantity" name="quantity" value={requestForm.qty}className="w-full border rounded border-blue-500 px-2 py-1 focus:outline-blue-800" onChange={(e)=> setRequestFrom({...requestForm, qty: e.target.value})}/>
          </label>
          <label>
            <span className="font-semibold">Purpose :</span>
            <textarea value={requestForm.purpose} id="purpose" name="purpose" className="resize-none w-full border rounded border-blue-500 px-2 py-1 focus:outline-blue-800 h-24" onChange={(e)=> setRequestFrom({...requestForm, purpose: e.target.value})}></textarea>
          </label>
