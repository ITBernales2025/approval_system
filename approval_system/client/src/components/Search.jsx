...
export const Search = ({data, withPage,refetchSearch,refetchWithPage}) => {
  ...
  return (
    <div className=" flex justify-end mt-1 gap-5">
      <label >
        <input type="text" list="search" autoComplete="searchTicket" id="searchTicket" name="searchTicket" placeholder="Search" value={searchData} onChange={handleSearch} className="border-2 rounded py-1 px-2 xs:text-[0.5em] lg:text-base xs:w-32 lg:w-48" />
      </label>
      <datalist id="search">
        {
      ...
      <label >
        <select className="border-2 rounded py-1 px-2 xs:text-[0.5em] lg:text-base xs:w-32 lg:w-48" value={searchStatus} onChange={handleSearchStatus} autoComplete="searchStatus" id="searchStatus" name="searchStatus">
