...
// add new ticket
export const addRequest = asyncHandler(async (req, res, next) => {
  ...
  try {
    const newTicket = await Ticket.create({
      employee_name, position, department , request, email, purpose, qty, ticket_no: ticketNoGenerator(), user: id, branch: user.branch
    })
    // oam.email
    if(request === "SIM") {
...
export const updateTicketOpen = asyncHandler(async(req, res, next) => {
...
export const findUserTickets = asyncHandler(async (req, res, next) => {
  const {page, userId} = req.params
  try {
    const tickets = await Ticket.find({user: userId}).limit(10).skip((page - 1) * 10).sort({createdAt: -1})
...
export const userTickets = asyncHandler(async(req, res, next) => {
  const {id} = req.params
  try {
    const userTicket = await Ticket.find({user: id})
...
export const aomTickets = asyncHandler(async(req, res, next) => {
  const {id} = req.params
  const user = await User.findById(id)
  if(!user) {
  ...
  const aomDeptsArray = []
  const depts = await Department.find({aom: user._id})
  Array.from(depts).forEach((element)=> {
    aomDeptsArray.push(element.name)
  })
  
  try {
    const tickets = await Ticket.find({department: {$in: aomDeptsArray}}).populate('user')
    return res.status(200).json(tickets)
  } catch (error) {
    next(error)
  }
...
export const aomTicketsWithPage = asyncHandler(async(req, res, next) => {
  const {page,userId} = req.params
  ...
  try {
    const tickets = await Ticket.find({department: {$in: aomDeptsArray}}).limit(10).skip((page - 1) * 10).sort({createdAt: -1}).populate('user')
    return res.status(200).json(tickets)
  } catch(err) {
    next(err)
  }
...
//rejecting declining
export const ticketDeclined = asyncHandler(async(req, res, next) => {
  const {id} = req.params
  const {comment, name} = req.body
  const ticket = await Ticket.findById(id).populate('user')
  if(!ticket) return res.status(404).json({message: "Ticket not found"})
  mail("robert.ramos07281021@gmail.com","Reject").catch(console.error)
  try {
...
export const userTicketReattempt = asyncHandler(async(req, res, next) => {
  const {id} = req.params
  try {
...
//user ticket update 
export const userTicketUpdate = asyncHandler(async(req, res, next) => {
  const {id} = req.params
  try {
...
//user delete ticket
export const userDeleteTicket = asyncHandler(async(req, res, next) => {
  const {id} = req.params
  try {
...
export const updateLastApproval = asyncHandler(async(req, res) => {
  ...
  mail("berto.07281021@gmail.com","Receiving").catch(console.error)
  try {
    if(comment.trim() !== "") {
...
export const updateSemiApproval = asyncHandler(async (req,res) => {
  ...
  if(ticket.request === "SIM") {
    mail("berto.07281021@gmail.com","Verification").catch(console.error)
  } else {
...
export const updateFraud = asyncHandler(async(req, res) => {
  ...
  mail("berto.07281021@gmail.com","Fraud").catch(console.error)
  try {
...
export const search = asyncHandler(async(req, res) => {
  ...
  if(user.type === "AOM") {
    const depts = [];
    const aomAccount = await User.findById(user._id)
    const aomCampaign = await Department.find({aom: aomAccount._id})
    Array.from(aomCampaign).forEach((element) => {
